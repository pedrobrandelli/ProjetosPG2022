	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (size_t i = 0; i < 5; i++)
		{
			for (size_t j = 0; j < 5; j++)
			{
				...
				model = glm::mat4(1);
				model = glm::translate(model, glm::vec3((160.f * j) + 80.f, (160.f * i) + 80.f, 0.0f));
				model = glm::scale(model, glm::vec3(160.0f, 160.f, 1.0f));
				shader.setMat4("model", glm::value_ptr(model));

				shader.setVec4("inputColor", cores[i][j][0], cores[i][j][1], cores[i][j][2], 1.0f); //enviando cor para variável uniform inputColor
